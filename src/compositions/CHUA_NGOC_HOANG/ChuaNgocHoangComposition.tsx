...
const ImageLayer: React.FC<{
  ...
  // Entrance scale/fade for the NEW image appearing
  const entranceProgress = springProgress(
    slideFrame,
    fps,
    isTransitioning ? TRANSITION_DURATION : 0,
    20,
  );

  const currentImage = SLIDES[slideIndex]?.image ?? SLIDES[0].image;
  const prevImage =
    slideIndex > 0 ? (SLIDES[slideIndex - 1]?.image ?? SLIDES[0].image) : null;

  // CSS 3D page-turn for outgoing image (rotates around right edge, folds to the left)
  const outgoingRotateY = interpolate(flipProgress, [0, 1], [0, 180]);
  // CSS 3D page-turn for incoming image (starts folded on the left, opens up)
  const incomingRotateY = interpolate(flipProgress, [0, 1], [-180, 0]);
  ...
  return (
    ...
    >
      {/* Outgoing image (page flips away during transition) */}
      {isTransitioning && prevImage && flipProgress < 1 && (
        <div
          style={{
            position: "absolute",
            transformOrigin: "right center",
            transform: `rotateY(${outgoingRotateY}deg)`,
      ...
      <div
        style={{
          ...
          transform: isTransitioning
            ? `rotateY(${incomingRotateY}deg)`
            : `none`,
          transformStyle: "preserve-3d",
          backfaceVisibility: "hidden",
          opacity: isTransitioning
            ? 1
            : interpolate(entranceProgress, [0, 0.4], [0, 1]),
          zIndex: 1,
        }}
      >
        <div style={imageStyle}>
